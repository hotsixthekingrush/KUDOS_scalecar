...
# 단위는 모두 meter.
# 예:
#   dx = +0.10  -> +X 방향으로 10 cm 이동
#   dx = -0.10  -> -X 방향으로 10 cm 이동
#   dy = +0.10  -> +Y 방향으로 10 cm 이동
#   dy = -0.10  -> -Y 방향으로 10 cm 이동
#
# VS Code에서 Ctrl+F로 "cone_14"처럼 검색하면
# 원하는 라바콘의 위치 보정값을 바로 찾을 수 있다.
#
# 기본 곡선 형상은 건드리지 않고 각 cone에 offset만 더하는 방식이라,
# 대회장 전체 geometry가 바뀌어도 개별 cone 미세조정값을 유지하기 쉽다.
#
CONE_OFFSETS = {
    "cone_01": {"dx": 0.00, "dy": 0.00},
    "cone_02": {"dx": 0.00, "dy": 0.00},
    "cone_03": {"dx": 0.00, "dy": 0.00},
    "cone_04": {"dx": 0.00, "dy": 0.00},
    "cone_05": {"dx": 0.00, "dy": 0.00},
    "cone_06": {"dx": 0.00, "dy": 0.00},
    "cone_07": {"dx": 0.00, "dy": 0.00},
    "cone_08": {"dx": 0.00, "dy": 0.00},
    "cone_09": {"dx": 0.00, "dy": 0.00},
    "cone_10": {"dx": 0.00, "dy": 0.00},
    "cone_11": {"dx": 0.00, "dy": 0.00},
    "cone_12": {"dx": 0.00, "dy": 0.00},
    "cone_13": {"dx": 0.00, "dy": 0.00},
    "cone_14": {"dx": 0.00, "dy": 0.00},
    "cone_15": {"dx": 0.00, "dy": 0.00},
    "cone_16": {"dx": 0.00, "dy": 0.00},
    "cone_17": {"dx": 0.00, "dy": 0.00},
    "cone_18": {"dx": 0.00, "dy": 0.00},
    "cone_19": {"dx": 0.00, "dy": 0.00},
    "cone_20": {"dx": 0.00, "dy": 0.00},
    "cone_21": {"dx": 0.00, "dy": 0.00},
    "cone_22": {"dx": 0.00, "dy": 0.00},
    "cone_23": {"dx": 0.00, "dy": 0.00},
    "cone_24": {"dx": 0.00, "dy": 0.00},
}
...
def create_bottom_straight(world):
    ...
    주차구역 입구를 막지 않도록 upper line은 x=6.80에서 끝난다.
    lower line은 기존 하단 도로 끝(BOTTOM_X_END)까지 유지한다.
    """
...
def create_parking_zone(world):
    """
    주차구역 선과 벽을 한 곳에서 생성한다.

    기존 apply_final_geometry_fixes()가 만들던 최종 형상을
    여기에서 직접 생성해 후처리 중복을 없앴다.
    """

    # 최종 주차선 형상.
    # parking_shape_1 / 3은 기존 후처리 결과와 동일하게 0.83 m 길이.
    segments = [
        ((6.20, -3.35), (6.20, -2.52)),  # parking_shape_1
        ((6.20, -2.72), (7.18, -2.72)),  # parking_shape_2
        ((7.18, -2.52), (7.18, -3.35)),  # parking_shape_3
        ((7.18, BOTTOM_Y_TOP), (7.62, BOTTOM_Y_TOP)),  # parking_shape_4
        ((7.62, BOTTOM_Y_TOP), (7.62, BOTTOM_Y_BOTTOM)),  # parking_shape_5
    ]
...
def create_cone(world, x, y, cone_id):
    """
    단일 mesh 라바콘.

    개별 위치는 파일 상단 CONE_OFFSETS에서 수정한다.

    예:
        "cone_01": {"dx": 0.10, "dy": 0.20}
    ...
    cone_name = f"cone_{cone_id:02d}"

    offset = CONE_OFFSETS.get(
        cone_name,
        {
            "dx": 0.0,
            "dy": 0.0
...
def create_curve_cones(world):
    ...
    각 위치는 파일 상단 CONE_OFFSETS에서 개별 조정.
    """
